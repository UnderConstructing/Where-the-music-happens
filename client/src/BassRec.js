...
export default function BassRec(props) {
  ...
  bassArrayRec.push (
    ...
    function activateBassNote(e) {
        if (bassArrayRec[e.target.getAttribute("row") - 1][(e.target.id - 1)].isActive === false) {
            bassArrayRec[e.target.getAttribute("row") - 1][(e.target.id - 1)].isActive = true
        }
        else {
            bassArrayRec[e.target.getAttribute("row") - 1][(e.target.id - 1)].isActive = false
        }
    }
    return (
        <div key="bass" >
            {bassArrayRec.map((row, i) => (
